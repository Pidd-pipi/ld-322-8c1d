...
export interface Sensor { id: number; greenhouseId: number; name: string; type: SensorType; unit: string; status: string; calibrationOffset: number; threshold?: Threshold; }
export interface Greenhouse { id: number; name: string; location: string; area: number; sensors: Sensor[]; devices: Device[]; }
export interface Reading { id: number; sensorId: number; value: number; calibratedValue: number; recordedAt: string; sensor: Sensor; }
